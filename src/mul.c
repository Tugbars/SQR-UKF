        ...
        uint16_t row_b, uint16_t column_b)
{
    ...
    const size_t max_n_panels = (Nc + 7) / 8;
    const size_t max_Bp_elems = Kc * max_n_panels * 8;
    float *Bp = (float *)linalg_aligned_alloc(LINALG_DEFAULT_ALIGNMENT, max_Bp_elems * sizeof(float));
    if (!Bp)
        return -ENOMEM;
    const size_t max_n_stripes = (Mc + 7) / 8;
    const size_t max_Ap_elems = Kc * max_n_stripes * 8;
            ...
            for (size_t i0 = 0; i0 < M; i0 += Mc)
            {
                ...
                for (; i + 7 < ib_tile; i += 8)
                {
                    ...
                    for (size_t p = 0, j = j0; p < n_panels_tile; ++p, j += 8, panel_off += Kblk * 8)
                    {
                        ...
                        __m256i mask = avx2_tailmask((int)jb);
                        if (kk == 0)
                        {
